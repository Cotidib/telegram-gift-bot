            ...
            if (storage.AskMixedCompleted)
            {
                if (storage.AnswersMixedQuestions.ContainsValue("si"))
                {
                    ...
                    if (storage.SpecificCategoriesSelected.Count > 0)
                    {
                        ...
                        output.SendMessageAsync("Se ha finalizado la fase de seleccion de preguntas especificas", request.RequestId);
                    }
                ...
                {
                    output.SendMessageAsync("¿No le gusta nada? ¡Bueno, intenemos de nuevo!", request.RequestId);
                    //return  null;
